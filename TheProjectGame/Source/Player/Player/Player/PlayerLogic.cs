...
namespace Player
{
    public class PlayerLogic : Messages.PlayerInterfaces.IPlayerLogic
    {
        ...
        private List<Xsd2.Player> _teamMembers;
        ...
        #endregion
        public List<Xsd2.Player> TeamMembers { get { return _teamMembers; } set { _teamMembers = value; } }
        public Piece GetPiece { get { return _pieceTaken; } }
        public List<TaskField> GetTaskFields {
            get
            {
                List<TaskField> fields = new List<TaskField>();
                foreach(List<Field> l in _board)
                {
                    foreach(var f in l)
                    {
                        if(f is TaskField)
                        {
                            fields.Add(f as TaskField);
                        }
                    }
                }
                return fields;
        ...
        public List<GoalField> GetGoalFields
        {
            get
            {
                List<GoalField> fields = new List<GoalField>();
                foreach (List<Field> l in _board)
                {
                    foreach (var f in l)
                    {
        ...
        {
            _teamMembers = gameMessage.Players.ToList<Xsd2.Player>();
